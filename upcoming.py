...
def _get_df_all_leagues(df_current: pd.DataFrame) -> pd.DataFrame:
    ...
    **normalizza le colonne** in Home/Away/country/Stagione/Quote.
    ...
    if callable(_get_global_source_df):
        try:
            df0, info = _get_global_source_df()
            ...
            if origin == "supabase" and url and callable(_read_parquet_filtered):
                try:
                    df_src = _read_parquet_filtered(url, league="", seasons=seasons)
                except Exception as e:
                    st.warning(f"Bypass Supabase fallito: {e}")
            if df_src is None and isinstance(df0, pd.DataFrame) and not df0.empty:
                df_src = df0
        except Exception as e:
...
COMPETITION_HINTS = {
    ...
    "eredivisie": "netherlands", "eerste": "netherlands", "keuken": "netherlands",
    "seriea": "italy", "serieb": "italy", "coppa": "italy",
...
def render_upcoming(df_current: pd.DataFrame, db_selected_label: str, run_pre_match_cb):
    ...
    for _, r in dfu.iterrows():
        ...
        records.append({
            ...
            "Over 1.5": ov15, "Over 2.5": ov25, "Over 3.5": ov35, "BTTS": btts,
        })
