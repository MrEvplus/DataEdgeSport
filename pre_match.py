...
def run_pre_match(df: pd.DataFrame, db_selected: str):
    ...
    # ======== Tabs (compresi Live e Correct Score) ========
    ...
    with tab_stats:
        render_team_stats_tab(
            df_league_all,     # campionato filtrato corrente
            league,
            squadra_casa,
            squadra_ospite,
        )
...
# =========================
#  PROTRADER — KPI PLUS (4 blocchi + precisione SOT→Gol)
#  Da incollare in pre_match.py (sezione helper UI)
#  Poi chiama: render_macro_kpi_plus(df_filtrato, home_team, away_team)
# =========================
import re
import math
import numpy as np
...
def _to_dt(s):
    try:
        ...
        return pd.to_datetime(pd.Series(s).astype(str), errors="coerce")
...
def _mins_list(cell):
    """Parsa qualunque cella minuti gol in lista ordinata di int (1..130)."""
    if cell is None:
        return []
    s = str(cell)
    vals = []
...
def _first_goal_side_min(row):
    """Ritorna ('Home'|'Away', minuto) o (None, None) se non c'è primo gol."""
    # colonne canoniche se presenti
    ...
    # fallback su gh1/ga1 ecc. (se non ci sono le canoniche)
    ...
    if a1 < h1:
        return ("Away", a1)
    # pari minuto → ignora (tornare None evita bias)
    return (None, None)

def _team_df(df, team, side):
    """Filtra il df per una squadra in un dato side ('Home'|'Away')."""
    if side == "Home":
...
def _w_d_l(df, side):
    """Win/Draw/Loss rate dal punto di vista di side ('Home'|'Away')."""
    if df.empty:
        return (0.0, 0.0, 0.0)
    if side == "Home":
...
def _shots(df, side):
    """Ritorna dict con shots: for/against, sot_for/sot_against, pace media."""
    cols = {  # FT totals
...
def _reliability_stars(n):
    # solo numerosità → semplice e robusto
    if n >= 45: return "★★★★★"
    if n >= 30: return "★★★★☆"
    if n >= 20: return "★★★☆☆"
    if n >= 10: return "★★☆☆☆"
...
def _momentum_block(df, team, side, last_n=8):
    ...
    # ultime N (ordinando per Data se presente)
...
def _first_goal_tables(df, team_home, team_away):
    ...
    for _, r in work.iterrows():
        ...
        minute.append(m)
    work["first_side"] = who
    work["first_min"]  = minute

    # gruppi esiti dal punto di vista “1X2” del match (non della singola squadra)
    def _outcome(row):
    ...
    # (b) istogramma primo gol per finestre
...
# --------- 3) Stile & Ritmo (pace, conversion, save%) ---------
def _style_rhythm_block(df, team, side):
    ...
    # conversione: Gol / SOT For
    conv = np.nan
    if shots["sot_for"] and shots["sot_for"] > 0:
        conv = gf_sum / shots["sot_for"]  # 0..1

    # save% approx: 1 − GA / SOT Against
    savep = np.nan
    if shots["sot_against"] and shots["sot_against"] > 0:
        savep = 1.0 - (ga_sum / shots["sot_against"])

    # Pace: media (shots for + against) per match
    n = len(df_side)
    ...
    return {
        "pace": pace_pm,
        "conv": conv,         # precisione SOT→Gol (richiesta)
        "save": savep,
        "btts": btts,
        "over25": over25,
        "n": n
...
# --------- 4) Calibration 1X2 (quote → outcome) ---------
def _calibration_one(df, market: str, k_bins=5):
    """
    market: 'Home' | 'Draw' | 'Away'
    Ritorna tabella con bin quota, implied prob media, observed freq, N e Brier.
    """
...
def render_macro_kpi_plus(df_ctx: pd.DataFrame, home_team: str, away_team: str):
    """
    Mostra i 4 blocchi + conversione SOT→Gol sotto la tabella 1X2 già esistente.
    df_ctx: dataset già filtrato per campionato & stagioni (come nella tua pagina)
    """
    ...
    # 2) Primo Gol → Esito (Pressione & Rimonte)
    ...
    # 3) Stile & Ritmo (incl. precisione SOT→Gol)
    st.subheader("3) ⚙️ Stile & Ritmo (pace / conversione / save% / BTTS / Over2.5)")
    c1, c2 = st.columns(2)
    with c1:
...
# df_macro_ctx  → è il df filtrato per campionato & stagioni che già passi a Macro KPI
# home_team     → squadra selezionata come Casa
# away_team     → squadra selezionata come Trasferta

render_macro_kpi_plus(df_macro_ctx, home_team, away_team)
