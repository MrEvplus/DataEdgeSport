...
def run_pre_match(df: pd.DataFrame, db_selected: str):
    ...
    league = (db_selected or "").strip().str.upper()
    ...
    if "Stagione" in df.columns:
        ...
        with st.expander("⚙️ Filtro Stagioni", expanded=True):
            st.markdown("Seleziona profondità storica; **Intervallo rapido** parte dalla stagione più recente.")
            colA, colB = st.columns([2, 1])
            with colA:
                seasons_selected = st.multiselect("Seleziona stagioni (Manuale)", options=seasons_desc,
    ...
    # Tabs
    tab_1x2, tab_roi, tab_ev, tab_stats = st.tabs(["1X2", "ROI mercati", "EV storico squadre", "Statistiche squadre"])

    with tab_1x2:
        st.info("Tab 1X2 invariata (overview).")
        # ... (qui rimane la tua tab 1X2 già implementata) ...
        st.write("")

    with tab_roi:
        st.info("Tab ROI invariata: usa le quote condivise in alto.")
        st.write("")

    with tab_ev:
        st.info("Tab EV invariata: usa le quote condivise in alto.")
        st.write("")
